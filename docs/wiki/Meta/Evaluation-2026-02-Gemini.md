...
*   **Comprehensive and Structured:** The `docs/wiki` directory is exceptionally thorough, organized into logical categories (e.g., `Conventions`, `Decisions`, `TypeScript`).
...
*   **Modern IaC:** Utilizes OpenTofu for declarative, version-controlled infrastructure, with clear file-based modularity (e.g., `api_gateway.tf`, `aurora_dsql.tf`).
...
*   **Performance and Cost Optimized:** Leverages `arm64` (Graviton2) architecture for most Lambda functions, resulting in significant cost savings and performance improvements, while making pragmatic exceptions for `x86_64` where binary compatibility (e.g., `yt-dlp`, `ffmpeg`) requires it.
...
2.  **Prompt to enforce stricter dependency pinning:**
    > "Evaluate the feasibility and impact of enforcing stricter dependency pinning for all direct and indirect dependencies in `package.json`. Propose a strategy to convert all caret (`^`) ranges to exact versions for non-development dependencies. Include recommendations for automating dependency updates (e.g., using RenovateBot or Dependabot) to manage the increased maintenance overhead."
...
*   **Comprehensive Convention Enforcement:** The `src/mcp/validation/index.ts` module is the core of this system, exposing 29 distinct `ValidationRule`s. These rules leverage `ts-morph` for AST analysis to enforce a wide array of conventions, from coding style and architectural patterns (e.g., Vendor Encapsulation) to security policies and documentation standards.
...
*   **Formal Documentation of MCP Tools:** While `AGENTS.md` provides a high-level overview, detailed, auto-generated documentation (e.g., OpenAPI specification) for each MCP tool (input/output schemas, usage examples) would significantly enhance usability for external AI agents.
...
2.  **Prompt to add a new MCP tool for code generation:**
    > "Create a new MCP tool named `scaffold_lambda_function`. This tool should take parameters for `lambdaName`, `triggerType` (e.g., 'APIGateway', 'SQS', 'Scheduled'), and an optional `databaseTables` array. The tool should:
