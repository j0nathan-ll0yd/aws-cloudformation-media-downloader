...
1. **Test File Structure** - Complete Vitest test suite with describe blocks
2. **Mock Imports** - All necessary test helper imports
3. **Mock Setup** - Pre-configured mocks for:
   - Entity queries (using `vi.mock('#entities/queries')`)
...
└── test/
    └── index.test.ts     # Generated test (if doesn't exist)
```
...
vi.mock('#entities/queries', () => ({
  getFileDownload: vi.fn().mockResolvedValue(null),
  createFileDownload: vi.fn().mockResolvedValue({}),
  updateFileDownload: vi.fn().mockResolvedValue({})
}))

// Mock vendor wrapper
vi.mock('#lib/vendor/AWS/S3', () => ({
  uploadToS3: vi.fn<() => Promise<unknown>>().mockResolvedValue({})
}))
...
```typescript
// Detects: import {getFileDownload} from '#entities/queries'
// Generates:
vi.mock('#entities/queries', () => ({
  getFileDownload: vi.fn().mockResolvedValue(null),
  createFileDownload: vi.fn().mockResolvedValue({}),
...
vi.mock('#lib/vendor/AWS/S3', () => ({
  uploadToS3: vi.fn<() => Promise<unknown>>().mockResolvedValue({}),
  deleteObject: vi.fn<() => Promise<unknown>>().mockResolvedValue({})
}))
```
...
// Customized (specific)
uploadToS3: vi.fn<() => Promise<{s3Url: string}>>()
...
Then use Command Palette → "Tasks: Run Task" → "Scaffold Test"
...
```bash
❌ Test file already exists: src/lambdas/ListFiles/test/index.test.ts
