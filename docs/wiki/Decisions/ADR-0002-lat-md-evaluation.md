...
3. **Parallel documentation surfaces.** Adopting lat.md would create a 4th documentation layer (`lat.md/` alongside CLAUDE.md, AGENTS.md/wiki, graphrag) with no reduction in existing layers.
...
- The unique value (semantic search) is narrow for a ~30-Lambda codebase where agents don't struggle to find code — they struggle with stale references
...
3. lat.md gains institutional backing (e.g., Linux Foundation, major vendor adoption)

## Related
- Mantle convention checks (`~/.claude/principles/mantle-checks.md`) — C1-C65
