...
/**
 * Core fixture logging function (internal - prefer the direction-specific helpers below)
 */
function logFixture(
  ...
  const marker: FixtureMarker = {
    marker: 'FIXTURE',
    type,
    function: lambdaName || process.env.AWS_LAMBDA_FUNCTION_NAME || 'unknown',
    scenario,
    variant,
...
/**
 * Log incoming API request data as a fixture
 * @param data - The incoming request data (typically APIGatewayProxyEvent)
 * @param lambdaName - Optional Lambda name (defaults to process.env.AWS_LAMBDA_FUNCTION_NAME)
 *
 * @example
 * // Auto-detect Lambda name
 * logIncomingFixture(event)
 *
 * // Explicit Lambda name (for testing or non-standard contexts)
 * logIncomingFixture(event, "WebhookFeedly")
 */
export function logIncomingFixture(data: unknown, lambdaName?: string): void {
  const name = lambdaName || process.env.AWS_LAMBDA_FUNCTION_NAME || 'unknown'
  logFixture('APIRequest', name, 'incoming', data, name)
}

/**
 * Log outgoing API response data as a fixture
 * @param data - The outgoing response data (typically APIGatewayProxyResult)
 * @param lambdaName - Optional Lambda name (defaults to process.env.AWS_LAMBDA_FUNCTION_NAME)
 *
 * @example
 * logOutgoingFixture(response)
 */
export function logOutgoingFixture(data: unknown, lambdaName?: string): void {
  const name = lambdaName || process.env.AWS_LAMBDA_FUNCTION_NAME || 'unknown'
  logFixture('APIResponse', name, 'outgoing', data, name)
}

/**
 * Log internal operation data as a fixture (for specific scenarios/variants)
 * @param scenario - The business scenario (e.g., 'newFile', 'existingFile')
 * @param data - The operation data
 * @param lambdaName - Optional Lambda name (defaults to process.env.AWS_LAMBDA_FUNCTION_NAME)
 *
 * @example
 * // Log a specific scenario variant
 * logInternalFixture('newFile', event)
 * logInternalFixture('existingFile', event)
 */
export function logInternalFixture(scenario: string, data: unknown, lambdaName?: string): void {
  const name = lambdaName || process.env.AWS_LAMBDA_FUNCTION_NAME || 'unknown'
  logFixture('APIRequest', name, scenario, data, name)
}
```
...
export async function handler(event: CustomAPIGatewayRequestAuthorizerEvent, context: Context) {
  // Log incoming request - auto-detects Lambda name, logs to CloudWatch
  logIncomingFixture(event)
...
**Key Improvements in This API**:

1. **Direction in function name** → Self-documenting code
   - `logIncomingFixture()` vs. `logFixture('APIRequest', ...)`
   - Immediately clear what's being logged

2. **Auto-detection of Lambda name** → Less boilerplate
   - Reads `process.env.AWS_LAMBDA_FUNCTION_NAME` automatically
   - Optional override for testing: `logIncomingFixture(event, "CustomName")`

3. **Fewer parameters** → Cleaner call sites
   - Before: `logFixture('APIRequest', 'WebhookFeedly', 'incoming', event)` (4 params)
   - After: `logIncomingFixture(event)` (1 param)

4. **No redundant logging** → DRY principle
   - Fixture logging IS the CloudWatch log
   - No need for separate `logInfo('event <=', event)` calls
   - All request/response data captured once
...
- [ ] **Create logging helpers** (30 minutes)
  - [ ] Add `logFixture()` (internal) to `util/lambda-helpers.ts`
  - [ ] Add `logIncomingFixture()` helper
  - [ ] Add `logOutgoingFixture()` helper
  - [ ] Add `logInternalFixture()` helper
  - [ ] Define `FixtureMarker` interface
  - [ ] Add sanitization for sensitive fields

- [ ] **Add markers to API handlers** (1.5 hours)
  - [ ] WebhookFeedly: `logIncomingFixture(event)`, `logInternalFixture(variant, event)`, `logOutgoingFixture(result)`
  - [ ] ListFiles: `logIncomingFixture(event)`, `logOutgoingFixture(result)`
  - [ ] RegisterDevice: `logIncomingFixture(event)`, `logOutgoingFixture(result)`
  - [ ] RegisterUser: `logIncomingFixture(event)`, `logOutgoingFixture(result)`
  - [ ] LoginUser: `logIncomingFixture(event)`, `logOutgoingFixture(result)`
  - [ ] Remove redundant `logInfo('event <=', event)` and `logInfo('response =>', result)` calls
...
- [ ] **Update style guides**
  - [ ] Add `logIncomingFixture()` / `logOutgoingFixture()` / `logInternalFixture()` patterns to Lambda style guide
  - [ ] Document when to use each fixture logging function
  - [ ] Document fixture naming convention
  - [ ] Update test style guide with integration test preference
  - [ ] Document that fixture logging replaces verbose event/response logging
...
// Mock fixture logging functions (not needed in unit tests, just suppress output)
jest.unstable_mockModule('../../../util/lambda-helpers', () => ({
  logIncomingFixture: jest.fn(),
  logOutgoingFixture: jest.fn(),
  logInternalFixture: jest.fn(),
  prepareLambdaResponse: jest.fn((config) => ({statusCode: config.statusCode, body: JSON.stringify(config.body)})),
  lambdaErrorResponse: jest.fn()
}))
...
- Document sanitization rules in `docs/wiki/Testing/Jest-ESM-Mocking-Strategy.md`
...
## Summary of API Design Improvements

The refined fixture logging API delivers significant ergonomic improvements:

### Before (Original Design)
```typescript
logInfo('event <=', event)  // Redundant
logFixture('APIRequest', 'WebhookFeedly', 'incoming', event)  // 4 parameters, manual Lambda name

const result = prepareLambdaResponse({statusCode: 200, body: {success: true}})
logFixture('APIResponse', 'WebhookFeedly', 'outgoing', result)  // Repetitive
logInfo('response =>', result)  // Redundant
```

**Issues**:
- ❌ Redundant logging (both `logInfo` and `logFixture`)
- ❌ Verbose API (4 parameters)
- ❌ Manual Lambda name (error-prone, duplicated)
- ❌ Direction not obvious from function name

### After (Refined Design)
```typescript
logIncomingFixture(event)  // Auto-detects Lambda name, 1 parameter

const result = prepareLambdaResponse({statusCode: 200, body: {success: true}})
logOutgoingFixture(result)  // Self-documenting, concise
```

**Benefits**:
- ✅ **Direction in function name** → `logIncomingFixture()` vs. `logFixture('APIRequest', ...)`
- ✅ **Auto-detection** → Reads `process.env.AWS_LAMBDA_FUNCTION_NAME` automatically
- ✅ **Fewer parameters** → 1-2 params instead of 4
- ✅ **No redundancy** → Fixture logging IS the CloudWatch log
- ✅ **Cleaner code** → Removed separate `logInfo()` calls
...
const result = prepareLambdaResponse({statusCode: 200, body: {success: true}})
logFixture('APIResponse', 'WebhookFeedly', 'outgoing', result)
logInfo('response =>', result)
```

**After**: 3 lines of logging
```typescript
logIncomingFixture(event)
...
**Impact**: 62% reduction in logging boilerplate
...
The refined API adds a fourth goal:

4. **Dramatically reduce logging boilerplate** with direction-specific helpers and auto-detection
