...
main() {
  # Configuration
  local DAYS_BACK=${1:-7}
  local OUTPUT_DIR=${2:-test/fixtures/raw}
  local START_TIME=$(($(date +%s) - (DAYS_BACK * 86400)))000
  ...
  for LAMBDA_NAME in "${LAMBDA_FUNCTIONS[@]}"; do
    ...
    if ! aws logs describe-log-groups --log-group-name-prefix "${LOG_GROUP}" --query 'logGroups[0].logGroupName' --output text 2> /dev/null | grep -q "${LOG_GROUP}"; then
      echo -e "  ${YELLOW}⚠${NC} Log group ${LOG_GROUP} not found, skipping..."
      continue
    fi
    ...
    if [ "${FIXTURE_COUNT}" -eq 0 ]; then
      echo -e "  ${BLUE}➜${NC} No fixtures found"
      rm -f "${OUTPUT_FILE}"
    else
      echo -e "  ${GREEN}✓${NC} Extracted ${FIXTURE_COUNT} fixtures → ${OUTPUT_FILE}"
    fi
  done
