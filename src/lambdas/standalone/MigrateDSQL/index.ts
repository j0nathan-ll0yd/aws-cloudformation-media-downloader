 ...
 *
 * Trigger: Direct Invocation (Terraform provisioner)
 * Input: MigrateDSQLInput with optional resetVersions
 * Output: MigrationResult with applied/skipped/errors
 *
...
export const handler = withObservability<MigrateDSQLInput, MigrationResult>({operationName: 'MigrateDSQL'}, async (input) => {
  ...
  logInfo('MigrateDSQL completed', result as unknown as Record<string, unknown>)
  return result
})
