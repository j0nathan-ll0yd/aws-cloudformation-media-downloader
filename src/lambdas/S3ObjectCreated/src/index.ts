 ...
 * @notExported
 */
async function getFileByFilename(fileName: string): Promise<FileRecord> {
  ...
  if (queryResponse.data && queryResponse.data.length > 0) {
    return queryResponse.data[0] as FileRecord
  } else {
 ...
 * @notExported
 */
async function getUsersOfFile(file: FileRecord): Promise<string[]> {
 ...
 * @notExported
 */
function dispatchFileNotificationToUser(file: FileRecord, userId: string) {
