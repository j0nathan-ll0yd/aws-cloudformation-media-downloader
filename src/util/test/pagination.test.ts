...
describe('scanAllPages', () => {
  ...
  it('should paginate through multiple pages', async () => {
    const mockScan = jest.fn<(cursor?: string) => Promise<{data: string[]; cursor: string | null}>>().mockResolvedValueOnce({
      data: ['page1-item1', 'page1-item2'],
      cursor: 'cursor1'
    }).mockResolvedValueOnce({data: ['page2-item1', 'page2-item2'], cursor: 'cursor2'}).mockResolvedValueOnce({data: ['page3-item1'], cursor: null})
  ...
  it('should return empty array when no data', async () => {
    const mockScan = jest.fn<(cursor?: string) => Promise<{data: string[]; cursor: string | null}>>().mockResolvedValue({data: [], cursor: null})
  ...
  it('should work with complex objects', async () => {
    ...
    const mockScan = jest.fn<(cursor?: string) => Promise<{data: Device[]; cursor: string | null}>>().mockResolvedValueOnce({
      data: [
        {deviceId: '1', token: 'token1'},
        {deviceId: '2', token: 'token2'}
      ],
      cursor: 'next'
    }).mockResolvedValueOnce({data: [{deviceId: '3', token: 'token3'}], cursor: null})
