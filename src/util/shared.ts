...
import {FileRecord, UserDeviceRecord} from '#types/persistence-types'
import {Device, User} from '#types/domain-models'
...
export async function getUserDevices(userId: string): Promise<UserDeviceRecord[]> {
  ...
  return response.data as UserDeviceRecord[]
}
 ...
 */
export async function upsertFile(item: FileRecord) {
