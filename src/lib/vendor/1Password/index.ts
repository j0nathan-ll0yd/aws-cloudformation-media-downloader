...
async function getOnePasswordClient() {
  ...
  if (!process.env.OnePasswordServiceAccountToken) {
    throw new Error('OnePasswordServiceAccountToken environment variable is not set')
  }

  onePasswordClient = await sdk.createClient({
    auth: process.env.OnePasswordServiceAccountToken,
