...
export const SNS_ERRORS = {
  EndpointDisabled: {name: 'EndpointDisabledException', statusCode: 400, message: 'Endpoint is disabled'},
  InvalidParameter: {name: 'InvalidParameterException', statusCode: 400, message: 'Invalid parameter'},
  NotFound: {name: 'NotFoundException', statusCode: 404, message: 'Resource not found'},
  Throttled: {name: 'ThrottledException', statusCode: 429, message: 'Rate exceeded'}
} as const
...
export const SQS_ERRORS = {
  QueueDoesNotExist: {name: 'QueueDoesNotExist', statusCode: 400, message: 'The specified queue does not exist'},
  MessageNotInflight: {name: 'MessageNotInflight', statusCode: 400, message: 'The message referred to is not in flight'},
  Throttled: {name: 'OverLimit', statusCode: 429, message: 'The maximum number of in-flight messages is reached'}
} as const
...
export const EVENTBRIDGE_ERRORS = {
  ResourceNotFound: {name: 'ResourceNotFoundException', statusCode: 404, message: 'Rule not found'},
  Throttled: {name: 'ThrottledRequest', statusCode: 429, message: 'Request rate exceeded'},
  InternalError: {name: 'InternalException', statusCode: 500, message: 'Internal service error'}
} as const

/**
 * Creates an AWS SDK-style error object
 */
export function createAwsError(errorType: {name: string; statusCode: number; message: string}, details?: Record<string, unknown>): Error {
  const error = new Error(errorType.message) as Error & {name: string; $metadata: {httpStatusCode: number}; Code: string; $fault: string}
